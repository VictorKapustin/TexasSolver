...
DiscountedCfrTrainableHF::DiscountedCfrTrainableHF(vector<PrivateCards> *privateCards,
    ...
    this->r_plus_local = vector<float>(this->action_number * this->card_number, 0.0f);
}
...
void DiscountedCfrTrainableHF::getcurrentStrategyInPlace(float* buffer) {
    ...
    for (int action_id = 0; action_id < action_number; action_id++) {
        for (int private_id = 0; private_id < this->card_number; private_id++) {
            ...
            this->r_plus_local[index] = val;
            this->r_plus_sum[private_id] += max(0.0f, val);
        }
    }
    ...
    for (int action_id = 0; action_id < action_number; action_id++) {
        for (int private_id = 0; private_id < this->card_number; private_id++) {
            ...
            if (this->r_plus_sum[private_id] != 0) {
                buffer[index] = max(0.0f, this->r_plus_local[index]) / this->r_plus_sum[private_id];
            } else {
                buffer[index] = 1.0f / (this->action_number);
            }
        }
    }
}
...
void DiscountedCfrTrainableHF::updateRegretsInPlace(const float* regrets, int iteration_number, const float* reach_probs) {
    ...
    for (int action_id = 0; action_id < action_number; action_id++) {
        for (int private_id = 0; private_id < this->card_number; private_id++) {
            ...
            this->r_plus_local[index] = this_r_plus_of_index;
        }
    }
    ...
    for (int action_id = 0; action_id < action_number; action_id++) {
        for (int private_id = 0; private_id < this->card_number; private_id++) {
            ...
            float current_strat;
            if (this->r_plus_sum[private_id] != 0) {
                current_strat = max(0.0f, this->r_plus_local[index]) / this->r_plus_sum[private_id];
            } else {
                current_strat = 1.0f / (this->action_number);
            }
            this->cum_r_plus[index] = (half)((float)this->cum_r_plus[index] * this->theta + current_strat * strategy_coef);
        }
    }
}
