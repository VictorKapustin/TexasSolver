    ...
    foreach ($threads in $ThreadCounts) {
        ...
        foreach ($hf in $HalfFloatModes) {
            ...
            foreach ($run in 1..$RunsPerThread) {
                ...
                Write-Host ("[{0}] Starting run {1}/{2} | threads={3} | pinning={4} | hf={5} | affinity={6}" -f (Get-Date -Format "HH:mm:ss"), $run, $RunsPerThread, $threads, $pinningProfile, $hf, $affinityLabel)
            ...
            $row = [pscustomobject]@{
                ...
                strategy_fetch_ms    = [math]::Round([double]$lastSummary.solver_profile.strategy_fetch, 2)
                regret_update_ms     = [math]::Round([double]$lastSummary.solver_profile.regret_update, 2)
            ...
            Write-Host ("[{0}] Finished run {1}/{2} | threads={3} | pinning={4} | hf={5} | solve={6} ms | iter={7} | exploitability={8:N6} | fetch={9} ms | regret={10} ms" -f (Get-Date -Format "HH:mm:ss"), $run, $RunsPerThread, $threads, $pinningProfile, $hf, $row.solve_wall_ms, $row.iteration, $row.exploitability, $row.strategy_fetch_ms, $row.regret_update_ms)
        }
    }
}
}
...
| Pinning | Threads | HF | Min | Max | Average | Median |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
$(foreach ($a in $Aggregate) {
    "| {0} | {1} | {2} | {3} | {4} | {5} | {6} |" -f $a.pinning_profile, $a.threads, $a.hf_mode, $a.min_solve_wall_ms, $a.max_solve_wall_ms, $a.avg_solve_wall_ms, $a.median_solve_wall_ms
})

## Performance Efficiency (Median)

| Pinning | Threads | HF | Iterations | Exploitability | Fetch (ms) | Regret (ms) | BR (ms) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
$(foreach ($a in $Aggregate) {
    "| {0} | {1} | {2} | {3} | {4:N6} | {5:N2} | {6:N2} | {7:N2} |" -f $a.pinning_profile, $a.threads, $a.hf_mode, $a.median_iteration, $a.median_exploitability, $a.median_strategy_fetch_ms, $a.median_regret_update_ms, $a.median_best_response_ms
})

## Detailed Resource Usage (Median)

| Pinning | Threads | HF | Alloc MB | River Lookup (ms) | River Hit Rate | Lock Wait (ms) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
$(foreach ($a in $Aggregate) {
    "| {0} | {1} | {2} | {3:N2} | {4:N2} | {5:P2} | {6:N2} |" -f $a.pinning_profile, $a.threads, $a.hf_mode, $a.median_allocator_mb, $a.median_river_lookup_ms, $a.median_river_hit_rate, $a.median_river_lock_wait_ms
})
...
foreach ($pinningProfile in $PinningProfiles) {
    foreach ($threads in $ThreadCounts) {
        foreach ($hf in $HalfFloatModes) {
            $group = $summaryRows | Where-Object { $_.pinning_profile -eq $pinningProfile -and $_.threads -eq $threads -and $_.hf_mode -eq $hf }
        ...
        $aggregate.Add([pscustomobject]@{
            ...
            median_strategy_fetch_ms = [math]::Round((Get-Median ($group.strategy_fetch_ms)), 2)
            median_regret_update_ms = [math]::Round((Get-Median ($group.regret_update_ms)), 2)
...
$summaryRows | Sort-Object pinning_profile, threads, hf_mode, run | Format-Table pinning_profile, threads, hf_mode, run, iteration, exploitability, solve_wall_ms, strategy_fetch_ms, regret_update_ms -AutoSize

Write-Host ""
Write-Host "Aggregate summary:"
$aggregate | Sort-Object pinning_profile, threads, hf_mode | Format-Table pinning_profile, threads, hf_mode, runs, median_solve_wall_ms, avg_solve_wall_ms, median_iteration, median_strategy_fetch_ms, median_regret_update_ms -AutoSize
